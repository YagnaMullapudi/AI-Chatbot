  ...
  if (!import_fs.default.existsSync(DB_PATH)) {
    const initialDB = {
      ...
      analytics: {
        ...
        aiTokenUsage: [
          { date: "2026-07-10", input: 24500, output: 48e3 },
          { date: "2026-07-11", input: 28900, output: 52400 },
          { date: "2026-07-12", input: 35e3, output: 68100 },
          { date: "2026-07-13", input: 41200, output: 82e3 },
...
function searchKnowledgeBase(query, documents) {
  ...
  const searchWords = query.toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter((w) => w.length > 2);
...
function logAudit(userId, email, action, details, type = "info") {
  const newLog = {
...
app.put("/api/chats/:id", (req, res) => {
  ...
  if (title !== void 0) chat.title = title;
  if (isPinned !== void 0) chat.isPinned = isPinned;
  if (folder !== void 0) chat.folder = folder;
...
app.post("/api/documents/upload", (req, res) => {
  ...
  db.analytics.knowledgeBaseSizeKb = Math.round(
    db.documents.reduce((acc, d) => acc + d.size / 1024, 0)
  );
  saveDB(db);
...
app.post("/api/chats/:id/messages", async (req, res) => {
  ...
  let dailyVolumeDay = db.analytics.dailyVolume.find((d) => d.date === todayStr);
  if (!dailyVolumeDay) {
  ...
  if (hasGrounding) {
    const contextText = citationsMatched.map((c, idx) => `[Source ${idx + 1}: ${c.docName}]
"${c.snippet}"`).join("\n\n");
...
3. Show clear, professional bulleted citations at the very end of your response under a '## Citations' header.`;
  ...
  const conversationContents = chat.messages.map((m) => {
    return {
...
`);
      ...
      const baseResponse = hasGrounding ? `According to the uploaded company records in **${citationsMatched[0].docName}**, our official policy is clearly defined [Source 1]. 
...
Is there any additional section of standard policies you would like me to retrieve?` : `Greetings! I am simulated in **Offline Enterprise Demo Mode** because a valid GEMINI_API_KEY was not supplied in the workspace secrets panel. However, here is how the enterprise chat responds to your query: "${text}".
...
`);
    ...
    let usageDay = db.analytics.aiTokenUsage.find((d) => d.date === todayStr);
    if (!usageDay) {
    ...
    if (aiClient) {
      ...
      aiText = hasGrounding ? `According to the uploaded company records in **${citationsMatched[0].docName}**, our official policy is clearly defined [Source 1]. 
...
Is there any additional section of standard policies you would like me to retrieve?` : `Greetings! This is a mock AI response representing: "${text}". Please provide a GEMINI_API_KEY in the secrets menu to connect to the actual model.`;
    }
